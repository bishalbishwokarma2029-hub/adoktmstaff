...
export default function LotDetailPage() {
  ...
  const entries = useMemo(() => {
    if (!lotNo) return [];
    const decoded = decodeURIComponent(lotNo);
    return allEntries.filter(e => e.lotNo === decoded);
  }, [allEntries, lotNo]);
  ...
  return (
    <div className="p-4">
      <div className="flex items-center gap-3 mb-4">
        <Button variant="ghost" size="sm" onClick={() => navigate('/lotwise')}><ArrowLeft className="h-4 w-4 mr-1" /> Back</Button>
        <h2 className="text-xl font-bold">LOT: {lotNo ? decodeURIComponent(lotNo) : ''}</h2>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="sticky top-0 z-20">
            <tr>
              ...
              <th className="p-2 text-left font-bold">Container</th>
          ...
          <tbody>
            {entries.map(e => (
              <tr key={e.id} className={`border-b hover:bg-accent/50 ${getDestinationClass(e.destination) ? 'text-destructive' : ''}`}>
                ...
                <td className="p-2 font-bold">{e.container}</td>
                <td className="p-2 font-bold">{e.remarks}</td>
              </tr>
            ))}
            {entries.length === 0 && <tr><td colSpan={selectMode ? 13 : 12} className="p-8 text-center text-muted-foreground font-bold">No entries found</td></tr>}
          </tbody>
        </table>
      </div>
